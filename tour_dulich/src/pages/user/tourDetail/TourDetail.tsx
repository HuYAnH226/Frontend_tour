...
export default function TourDetail() {
  ...
  const handleBooking = async () => {
    ...
    if (!maUser || !id) {
      alert("Thiếu thông tin người dùng hoặc mã tour. Vui lòng đăng nhập lại.");
      return;
    }
    if (!bookingData.diemDi || bookingData.soKhach < 1) {
      alert("Vui lòng nhập đầy đủ Số khách và Điểm đi.");
      return;
    }
    if (bookingData.soKhach > tour.soChoToiDa) {
      alert(
        `Số lượng khách tối đa cho tour này là ${tour.soChoToiDa}. Vui lòng kiểm tra lại.`
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `Đang giảm số lượng tour: ${id}, số người: ${bookingData.soKhach}`
      );
      ...
      if (!bookTourResponse.ok) {
        ...
        throw new Error(
          errorText || "Không thể đặt tour. Tour có thể đã hết chỗ."
        );
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <div className="relative h-96 bg-gradient-to-r from-blue-600 to-indigo-700">
        ...
        <div
          ...
          style={{ backgroundImage: `url(${tour.anhTour})` }}
        >
